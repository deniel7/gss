...
class Kategori_m extends MY_Model {
    ...
    public function __construct(){
        parent::__construct();
        parent::set_table('MS_MASTER','MS_CHILD');
    ...
    //    if($find_child == FALSE){
    //        $older = parent::get_many_by(array('MS_PARENT'=>'0'));
    //        //echo $this->db->last_query();
    //        foreach ($older as $parent) {
    //            $this->kategori_menu .= '<ul><li>'.anchor(site_url('/produk/'.$parent->ID), $parent->MS_DESC,'id="'.$parent->ID.'"');
    //            $this->get_kategori_menu(TRUE,$parent->ID);
    //            $this->kategori_menu .= '</li></ul>';
    //        }
    //    }else{
    //        
    ...
    //            foreach ($child as $item) {
    //                $this->kategori_menu .= '<ul><li>'.anchor(site_url('/store/kategori/'.$item->ID), $item->MS_DESC.'<span>v</span>','id="'.$item->ID.'"');
    //                $this->get_kategori_menu(TRUE,$item->ID);
    //                $this->kategori_menu .= '</li></ul>';
    //            }
    //            $this->kategori_menu .= '</ul>';
    //        }
    ...
    //        $data['kode_cabang'] = $row->nama_cabang;
    //    }
    //    return $data;
    //}
    ...
    private function get_kategori_menu($find_child = FALSE,$parent_id = 'L6') {
        if($find_child == FALSE){
            $older = parent::get_many_by(array('MS_PARENT'=>'L6'));
