...
Class Jcrop {
	...
	public function show_form($action,$option = FALSE) {
		...
		if ($option == TRUE) {
			...
			';
			$input .= $this->form;
			$input .= '<button type="submit" id="'.$this->prefix.'save" name="'.$this->prefix.'save" class="input-submit" >Save Croped Image!</button>';
			$input.= '<button type="submit" id="'.$this->prefix.'cancel" name="'.$this->prefix.'cancel" class="input-submit" >Cancel</button>';
		} else {
			$enctype = ' enctype="multipart/form-data" ';
			$input = '<input type="file" name="'.$this->prefix.'picture" size="40"/><br /><br />';
			$input.= '<input type="submit" name="'.$this->prefix.'submit" value="Submit" class="input-submit">';
		}
