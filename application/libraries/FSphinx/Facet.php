 ...
 */
class Facet implements \IteratorAggregate, \Countable, DataSourceInterface
{
    /**
     ...
     *
     * Data sources are required to implement DataSourceInterface::FetchTerms()
     ...
     * @param DataSourceInterface $datasource Data source used to fetch Facet terms.
     */
    public function __construct($name, array $options = array(), DataSourceInterface $datasource = null)
    {
        $this->_name = $name;
        ...
        if ($datasource) {
            ...
            $this->attachDataSource($datasource, null);
        }
    ...
    {
        $this->_order_by = function ($v) use ($key) {
            return $v[$key];
        };
    ...
    {
        $this->_max_num_values = max(1, intval($max_num_values));
    }
    ...
    {
        $query = $this->prepare($query, $this->_sphinx);
     ...
     */
    public function prepare($query, FSphinxClient $sphinx)
    {
        ...
        $sphinx->saveOptions(
            array(
                ...
                'groupby', 'groupfunc', 'groupsort', 'groupdistinct', // modified by setGroupBy
    ...
    /**
     * DataSourceInterface method. Allows a Facet to act as a data source for term mapping.
     *
    ...
    public function fetchTerms(array $matches, array $options, \Closure $getter)
    {
        ...
        foreach ($matches as $match) {
            if (isset($match['attrs'][$attr])) {
                // The serialized attribute is in the form "1,a,2,b,3,c"
