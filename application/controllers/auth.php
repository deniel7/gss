...
class Auth extends CI_Controller {
	...
	function index()
	{
		...
		$data['recent'] = $this->m_home->tampil_recent();
		$data['city'] = $this->m_home->tampil_kota();
...
function registrasi(){
	$this->load->model('m_home','',TRUE);
		$data['recent'] = $this->m_home->tampil_recent();
		$this->data['recent'] = $this->m_home->tampil_recent();
		$data['city'] = $this->m_home->tampil_kota();
	
	
	
	//apabila user sudah login, maka halaman register tidak dapat ditampilkan
	if ($this->ion_auth->logged_in()){redirect('auth/index');}$data['title'] = "Halaman Registrasi";
	...
	//password wajib diisi, jumlah karakter lebih dari angka pengaturan min_password_lengthdan cocok dengan field password_confirm
	...
	if ($this->form_validation->run() == true){
		...
		$username = $this->input->post('username', true);$email = $this->input->post('email');$password = $this->input->post('password');
	...
	if ($this->form_validation->run() == true &&$this->ion_auth->register($username, $password, $email,$additional_data)){
		...
		//$this->session->set_flashdata('message',"Akun telah dibuat. Silakan cek email untuk aktivasi.");redirect('auth/login');
	
		//apabila akun langsung aktif tanpa perlu aktivasi via notifikasi email:	
		$this->session->set_flashdata('message', "Akun telah dibuat. Silakan login.");redirect('auth/login');
	
	}else
	        //apabila validasi salah atau membuka halaman pertama kali
	{
		...
		$main['username'] = array('name' => 'username','id' => 'username','type' => 'text','value' => $this->form_validation->set_value('username'),);
		$main['email'] = array('name' => 'email','id' => 'email','type' => 'text','value' => $this->form_validation->set_value('email'),);
		$main['password'] = array('name' => 'password','id' => 'password','type' => 'password','value' => $this->form_validation->set_value('password'),);
		$main['password_confirm'] = array('name' => 'password_confirm','id' => 'password_confirm','type' => 'password','value' => $this->form_validation->set_value('password_confirm'),);
		
		$data['content'] = $this->load->view('admin/register_v', $main, true);$this->load->view('template', $data);
	}
}
	...
	function login()
	{
		...
		//penambahan buat footer
		//$this->load->model('m_home','',TRUE);
		//$data['recent'] = $this->m_home->tampil_recent();
		//$this->data['recent'] = $this->m_home->tampil_recent();
		//$this->data['city'] = $this->m_home->tampil_kota();
	...
	{
	//apabila user sudah login
		if($this->ion_auth->logged_in()){$data['title'] = "Halaman User/Member";$data['content'] = $this->load->view('user_page_v', null, true);$this->load->view('template', $data);}else
		//apabila belum login
		{
		//redirect ke halaman login
		redirect('auth/login');}
	...
	function admin_page()
	{
		...
		if($this->ion_auth->is_admin()){$data['title'] = "Halaman Administrator";$data['content'] = $this->load->view('admin_page_v', null, true);$this->load->view('template', $data);}else
		//apabila bukan administrator
		{
	...
	public function reset_password($code = NULL)
	{
		...
		{  
			...
			else
			{
				...
				if ($this->_valid_csrf_nonce() === FALSE || $user->id != $this->input->post('id_user')) 
				{
